...
const WithdrawEditPage = props => {
  ...
  const handleConfirm = () => {
    ...
    orderStore.updateWithDrawalOrder(payload)
      .then(res => {
        if (res?.responseCode === RESPONSE_CODE.SUCCESS) {
  ...
  useEffect(() => {
    ...
    if (!editingExecution) {
      ...
      let amount = Number(JSON.parse(editingExecution.executionData)?.WithdrawInfo?.Amount)
      let bankCode = JSON.parse(editingExecution.executionData)?.WithdrawInfo?.BankCode
      let accountName = JSON.parse(editingExecution.executionData)?.WithdrawInfo?.AccountName
  ...
  return (
    <>
      <Helmet>
        <title>Sửa yêu cầu rút tiền</title>
      </Helmet>
      <WithdrawPageWrapper>
        <MainBreadCrumb breadcrumbData={BREADCRUMB_DATA.WITHDRAW_EDIT} />
        <WhiteRoundedBox>
          <TitleInfoService>Sửa yêu cầu rút tiền</TitleInfoService>
          <Form
            size={'large'}
        ...
        <ConfirmInfoModal
          ...
          title={'Xác nhận sửa yêu cầu rút tiền'}
...
export default inject(
  ...
  'bankStore',
  'orderStore',
  'accountWalletStore')(observer(WithdrawEditPage))