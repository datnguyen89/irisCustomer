...
import { CollapsePanelTransferSingleWrapper, SpaceIcon } from './CollapsePanelTransferSingleStyled'
...
const CollapsePanelTransferSingle = props => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: record => ( // Tùy vào role render ra các nút tương ứng
        <SpaceIcon>
          ...
          {
            authenticationStore.checkMultipleRole([
              ROLES.REVIEWTRANSFERPERSONAL,
              ROLES.REVIEWTRANSFERENTERPRISE,
              ROLES.REVIEWTRANSFERMM,
            ]) &&
            record?.orderStatus === EXECUTION_ORDER_STATUS.WAIT_APPROVAL &&
            objFilterOrder.Status !== EXECUTION_ORDER_STATUS.IN_PROGRESS &&
            <>
          ...
          {
            authenticationStore.checkMultipleRole([
              ROLES.APPROVETRANSFERPERSONAL,
              ROLES.APPROVETRANSFERENTERPRISE,
              ROLES.APPROVETRANSFERMM,
            ]) &&
            (
              record?.orderStatus === EXECUTION_ORDER_STATUS.WAIT_APPROVAL ||
    ...
    },
    {
      ...
      align: 'center',
      render: record => (
        record?.executionStatus === EXECUTION_STATUS.SUCCESS &&
  ...
  const handleSubmitOtp = (e) => {
    switch (jwtDecode?.RoleType) {
      case ROLE_TYPE.REVIEW:
        ...
        switch (selectedExecution?.executionType) {
          case EXECUTION_TYPE_ID.TRANSFER_PERSONAL:
            orderStore.confirmReviewTransferPersonalExecution(payloadReview)
              .then(res => {
          ...
          case EXECUTION_TYPE_ID.TRANSFER_ENT:
            orderStore.confirmReviewTransferEntExecution(payloadReview)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    setVisibleOtp(false)
          ...
          case EXECUTION_TYPE_ID.TRANSFER_MM:
            orderStore.confirmReviewTransferMMExecution(payloadReview)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    setVisibleOtp(false)
      ...
      case ROLE_TYPE.APPROVE:
        ...
        switch (selectedExecution?.executionType) {
          case EXECUTION_TYPE_ID.TRANSFER_PERSONAL:
            orderStore.confirmTransferPersonalExecution(payload)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    setVisibleOtp(false)
                      ...
                      content:
                        <>
                          <RowCenterDiv margin={'16px 0'}>
                            ...
                            <ColorText fontSize={'1.6rem'} margin={'0 0 0 16px'}
                                       color={SUCCESS_COLOR}>
                              {res?.description}
                            </ColorText>
                          </RowCenterDiv>
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_ENT:
            orderStore.confirmTransferEntExecution(payload)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    setVisibleOtp(false)
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_MM:
            orderStore.confirmTransferMMExecution(payload)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    setVisibleOtp(false)
                  ...
                  case RESPONSE_CODE.PAYMENT_TRANSACTION_UNKNOW:
                    ...
                    Modal.info({
                      ...
                      content:
                        <>
                          <RowCenterDiv margin={'16px 0'}>
                            <InfoCircleOutlined style={{ fontSize: 32, color: SUCCESS_COLOR }} />
                            <ColorText fontSize={'1.6rem'} margin={'0 0 0 16px'}
                                       color={SUCCESS_COLOR}>
                              {res?.description}
                            </ColorText>
                  ...
                  case RESPONSE_CODE.PAYMENT_TRANSACTION_ERROR:
                    ...
                    Modal.info({
                      ...
                      content:
                        <>
                          <RowCenterDiv margin={'16px 0'}>
                            <CloseCircleOutlined style={{ fontSize: 32, color: SUCCESS_COLOR }} />
                            <ColorText fontSize={'1.6rem'} margin={'0 0 0 16px'}
                                       color={SUCCESS_COLOR}>
                              {res?.description}
                            </ColorText>
  ...
  const handleSubmitConfirm = (e) => {
    ...
    switch (jwtDecode?.RoleType) {
      ...
      case ROLE_TYPE.REVIEW:
        ...
        switch (selectedExecution?.executionType) {
          case EXECUTION_TYPE_ID.TRANSFER_PERSONAL:
            orderStore.reviewTransferPersonalExecution(payloadReview)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    notification.success({
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_ENT:
            orderStore.reviewTransferEntExecution(payloadReview)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    notification.success({
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_MM:
            orderStore.reviewTransferMMExecution(payloadReview)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    notification.success({
      ...
      case  ROLE_TYPE.APPROVE:
        ...
        switch (selectedExecution?.executionType) {
          case EXECUTION_TYPE_ID.TRANSFER_PERSONAL:
            orderStore.approveTransferPersonalExecution(payloadApprove)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    notification.success({
                      message: <ColorText fontSize={'20px'}
                                          color={SUCCESS_COLOR}>{SUCCESS_TITLE}</ColorText>,
                      description: res?.description,
                    })
                    reLoadData(groupTypeID)
                    setRowExpand([])
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_ENT:
            orderStore.approveTransferEntExecution(payloadApprove)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    notification.success({
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_MM:
            orderStore.approveTransferMMExecution(payloadApprove)
              .then(res => {
                switch (res?.responseCode) {
                  case RESPONSE_CODE.SUCCESS:
                    notification.success({
  ...
  const handleClickEditExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        history.push(PAGES.TRANSFER_WALLET_EDIT.PATH)
      },
      onCancel: () => {
      },
      content:
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        switch (e?.executionType) {
          case EXECUTION_TYPE_ID.TRANSFER_PERSONAL:
            orderStore.deleteTransferPersonalExecution(payload)
              .then(res => {
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_ENT:
            orderStore.deleteTransferEntExecution(payload)
              .then(res => {
                if (res?.responseCode === RESPONSE_CODE.SUCCESS) {
                  notification.success({
                    message: <ColorText fontSize={'20px'}
            ...
            break
          case EXECUTION_TYPE_ID.TRANSFER_MM:
            orderStore.deleteTransferMMExecution(payload)
              .then(res => {
                if (res?.responseCode === RESPONSE_CODE.SUCCESS) {
                  notification.success({
                    message: <ColorText fontSize={'20px'}
      ...
      <ConfirmExecutionModal
        ...
        description={
          jwtDecode?.RoleType === ROLE_TYPE.REVIEW
            ? `Bạn có đồng ý xác nhận yêu cầu chuyển tiền ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
            : jwtDecode?.RoleType === ROLE_TYPE.APPROVE ?
              `Bạn có đồng ý phê duyệt yêu cầu chuyển tiền ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
              : ''
        }
...
export default inject(
  ...
  'propertyStore',
  'commonStore',
  'paymentStore')(observer(CollapsePanelTransferSingle))