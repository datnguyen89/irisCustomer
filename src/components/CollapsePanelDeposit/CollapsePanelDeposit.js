...
import { CollapsePanelDepositWrapper, SpaceIcon } from './CollapsePanelDepositStyled'
...
const CollapsePanelDeposit = props => {
  ...
  const handleSubmitOtp = (e) => {
    switch (jwtDecode?.RoleType) {
      case ROLE_TYPE.REVIEW:
        ...
        orderStore.confirmReviewDepositExecution(payloadReview)
          .then(res => {
      ...
      case ROLE_TYPE.APPROVE:
        ...
        orderStore.confirmDepositExecution(payload)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                setVisibleOtp(false)
  ...
  const handleSubmitConfirm = (e) => {
    ...
    switch (jwtDecode?.RoleType) {
      ...
      case ROLE_TYPE.REVIEW:
        ...
        orderStore.reviewDepositExecution(payloadReview)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                notification.success({
      ...
      case  ROLE_TYPE.APPROVE:
        ...
        orderStore.approveDepositExecution(payloadApprove)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                notification.success({
  ...
  const handleClickEditExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        history.push(PAGES.DEPOSIT_EDIT.PATH)
      },
      onCancel: () => {
      },
      content:
  ...
  const handleClickDeleteExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        orderStore.deleteDepositExecution(payload)
          .then(res => {
  ...
  const getFee = (record) => {
    let payload = {
      ...
      Amount: JSON.parse(record?.executionData)?.DepositInfo?.Amount,
      BankCode: JSON.parse(record?.executionData)?.DepositInfo?.BankCode,
      BankID: JSON.parse(record?.executionData)?.DepositInfo?.BankID,
      BankServiceID: JSON.parse(record?.executionData)?.DepositInfo?.BankServiceID,
      ...
      <ExpandContent>
        <Descriptions
          ...
          <Descriptions.Item
            label={'Ngân hàng'}
            span={1}>
            {JSON.parse(record?.executionData)?.DepositInfo?.BankCode || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Số tài khoản ngân hàng'}
            span={1}>
            {JSON.parse(record?.executionData)?.DepositInfo?.BankAccount || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Số tài khoản ví'}
            span={1}>
            {JSON.parse(record?.executionData)?.DepositInfo?.AccountName || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Số tiền'}
            span={1}>
            {`${numberUtils.thousandSeparator(JSON.parse(record?.executionData)?.DepositInfo?.Amount)} đ` || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Phí giao dịch'}
            span={1}>
  ...
  // region side effect ========================
  useEffect(() => {
    console.log(rowsExpand)
  }, [rowsExpand])
  // endregion

  return (
    <CollapsePanelDepositWrapper>
      <div id='napasRoot' />
      <Table
        bordered={false}
        expandable={{
      ...
      <ConfirmExecutionModal
        ...
        description={
          jwtDecode?.RoleType === ROLE_TYPE.REVIEW
            ? `Bạn có đồng ý xác nhận yêu cầu nạp tiền ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
            : jwtDecode?.RoleType === ROLE_TYPE.APPROVE ?
              `Bạn có đồng ý phê duyệt yêu cầu nạp tiền ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
              : ''
        }
...
CollapsePanelDeposit.propTypes = {
  data: PropTypes.any,
...
export default inject('orderStore', 'authenticationStore', 'propertyStore', 'paymentStore')(observer(CollapsePanelDeposit))