...
import { CollapsePanelWithdrawMmWrapper, SpaceIcon } from './CollapsePanelWithdrawMmStyled'
...
const CollapsePanelWithdrawMm = props => {
    ...
    },
    {
      ...
      render: record => ( // Tùy vào role type render ra các nút tương ứng
        <SpaceIcon>
          ...
          {
            authenticationStore.checkRole(ROLES.REVIEWWITHDRAWMM) &&
            record?.orderStatus === EXECUTION_ORDER_STATUS.WAIT_APPROVAL &&
            objFilterOrder.Status !== EXECUTION_ORDER_STATUS.IN_PROGRESS &&
            <>
              <img
          ...
          {
            authenticationStore.checkRole(ROLES.APPROVEWITHDRAWMM) &&
            (
              record?.orderStatus === EXECUTION_ORDER_STATUS.WAIT_APPROVAL ||
  ...
  const handleSubmitOtp = (e) => {
    switch (jwtDecode?.RoleType) {
      case ROLE_TYPE.REVIEW:
        ...
        orderStore.confirmReviewWithdrawMMExecution(payloadReview)
          .then(res => {
      ...
      case ROLE_TYPE.APPROVE:
        ...
        orderStore.confirmWithdrawMMExecution(payload)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                setVisibleOtp(false)
  ...
  const handleSubmitConfirm = (e) => {
    ...
    switch (jwtDecode?.RoleType) {
      ...
      case ROLE_TYPE.REVIEW:
        ...
        orderStore.reviewWithdrawMMExecution(payloadReview)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                notification.success({
      ...
      case  ROLE_TYPE.APPROVE:
        ...
        orderStore.approveWithdrawMMExecution(payloadApprove)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                notification.success({
  ...
  const handleClickEditExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        history.push(PAGES.WITHDRAW_MM_EDIT.PATH)
      },
      onCancel: () => {
      },
      content:
  ...
  const handleClickDeleteExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        orderStore.deleteWithdrawMMExecution(payload)
          .then(res => {
  ...
  const getFee = (record) => {
    let payload = {
      TransferAccount: JSON.parse(record?.executionData)?.TransferInfo?.RelatedUser,
  ...
  return (
    <CollapsePanelWithdrawMmWrapper>
      <div id='napasRoot' />
      <Table
        bordered={false}
        expandable={{
      ...
      <ConfirmExecutionModal
        ...
        description={
          jwtDecode?.RoleType === ROLE_TYPE.REVIEW
            ? `Bạn có đồng ý xác nhận yêu cầu rút tiền TK tiền di động ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
            : jwtDecode?.RoleType === ROLE_TYPE.APPROVE ?
              `Bạn có đồng ý phê duyệt yêu cầu rút tiền TK tiền di động ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
              : ''
        }
...
CollapsePanelWithdrawMm.propTypes = {
  data: PropTypes.any,
...
export default inject(
  ...
  'authenticationStore',
  'propertyStore',
  'paymentStore')(observer(CollapsePanelWithdrawMm))