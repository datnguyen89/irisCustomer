...
import { CollapsePanelWithdrawWrapper, SpaceIcon } from './CollapsePanelWithdrawStyled'
...
const CollapsePanelWithdraw = props => {
    ...
    },
    {
      ...
      render: record => ( // Tùy vào role type render ra các nút tương ứng
        <SpaceIcon>
          ...
          {
            authenticationStore.checkRole(ROLES.REVIEWWITHDRAW) &&
            record?.orderStatus === EXECUTION_ORDER_STATUS.WAIT_APPROVAL &&
            objFilterOrder.Status !== EXECUTION_ORDER_STATUS.IN_PROGRESS &&
            <>
              <img
          ...
          {
            authenticationStore.checkRole(ROLES.APPROVEWITHDRAW) &&
            (
              record?.orderStatus === EXECUTION_ORDER_STATUS.WAIT_APPROVAL ||
  ...
  const handleSubmitOtp = (e) => {
    switch (jwtDecode?.RoleType) {
      case ROLE_TYPE.REVIEW:
        ...
        orderStore.confirmReviewWithdrawalExecution(payloadReview)
          .then(res => {
      ...
      case ROLE_TYPE.APPROVE:
        ...
        orderStore.confirmWithDrawalExecution(payload)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                setVisibleOtp(false)
  ...
  const handleSubmitConfirm = (e) => {
    ...
    switch (jwtDecode?.RoleType) {
      ...
      case ROLE_TYPE.REVIEW:
        ...
        orderStore.reviewWithDrawalExecution(payloadReview)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                notification.success({
      ...
      case  ROLE_TYPE.APPROVE:
        ...
        orderStore.approveWithDrawalExecution(payloadApprove)
          .then(res => {
            switch (res?.responseCode) {
              case RESPONSE_CODE.SUCCESS:
                notification.success({
  ...
  const handleClickEditExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        history.push(PAGES.WITHDRAW_EDIT.PATH)
      },
      onCancel: () => {
      },
      content:
  ...
  const handleClickDeleteExecution = (e) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        orderStore.deleteWithDrawalExecution(payload)
          .then(res => {
  ...
  const getFee = (record) => {
    let payload = {
      ...
      Amount: JSON.parse(record?.executionData)?.WithdrawInfo?.Amount,
      BankCode: JSON.parse(record?.executionData)?.WithdrawInfo?.BankCode,
      BankID: JSON.parse(record?.executionData)?.WithdrawInfo?.BankID,
      BankServiceID: JSON.parse(record?.executionData)?.WithdrawInfo?.BankServiceID,
      ...
      <ExpandContent>
        <Descriptions
          ...
          <Descriptions.Item
            label={'Ngân hàng'}
            span={1}>
            {JSON.parse(record?.executionData)?.WithdrawInfo?.BankCode || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Số tài khoản ngân hàng'}
            span={1}>
            {JSON.parse(record?.executionData)?.WithdrawInfo?.BankAccount || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Số tài khoản ví'}
            span={1}>
            {JSON.parse(record?.executionData)?.WithdrawInfo?.AccountName || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Số tiền'}
            span={1}>
            {`${numberUtils.thousandSeparator(JSON.parse(record?.executionData)?.WithdrawInfo?.Amount)} đ` || ''}
          </Descriptions.Item>
          <Descriptions.Item
            label={'Phí giao dịch'}
            span={1}>
      ...
      <ConfirmExecutionModal
        ...
        description={
          jwtDecode?.RoleType === ROLE_TYPE.REVIEW
            ? `Bạn có đồng ý xác nhận yêu cầu rút tiền ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
            : jwtDecode?.RoleType === ROLE_TYPE.APPROVE ?
              `Bạn có đồng ý phê duyệt yêu cầu rút tiền ${'\n'} (Mã yêu cầu ${selectedExecution?.executionID})`
              : ''
        }
...
CollapsePanelWithdraw.propTypes = {
  data: PropTypes.any,
...
export default inject(
  ...
  'authenticationStore',
  'propertyStore',
  'paymentStore')(observer(CollapsePanelWithdraw))